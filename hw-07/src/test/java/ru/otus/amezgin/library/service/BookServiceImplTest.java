...
class BookServiceImplTest {
    ...
    void checkingGetById() {
        ...
        doReturn(Optional.of(expectedGenre)).when(genreJPA).getById(GENRE_ID_1);
        Book expectedBook = new Book();
        expectedBook.setId(ID_ONE);
        expectedBook.setTitle(BOOK_TITLE_1);
        doReturn(Optional.of(expectedBook)).when(bookJPA).getById(ID_ONE);
    ...
    void checkingGetAll() {
        ...
        doReturn(expectedBookList).when(bookJPA).getAll();
    ...
    void checkingSave() {
        ...
        doReturn(book).when(bookJPA).save(book);
    ...
    void checkingUpdate() {
        ...
        doReturn(Optional.of(book)).when(bookJPA).getById(ID_ONE);
        Book expectedBook = bookService.getById(ID_ONE).get();
        expectedBook.setTitle(BOOK_TITLE_2);
        doReturn(expectedBook).when(bookJPA).update(expectedBook);
    ...
    void checkingDeleteById() {
        ...
        doAnswer(i -> books.remove(0)).when(bookJPA).deleteById(ID_ONE);
        doReturn(books).when(bookJPA).getAll();
